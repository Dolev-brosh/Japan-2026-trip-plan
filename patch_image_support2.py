...
if "const compressImage =" not in code:
    code = code.replace("export default function App() {", compress_util + "\nexport default function App() {")
...
if "const handleImageUpload =" not in code:
    code = code.replace("  const handlePasteDescription = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {", handle_image_upload_func + "\n  const handlePasteDescription = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {")
...
# For Ticket:
old_ticket_content_end = """                        </div>
                      )}
                    </div>
                  )}"""

if "ticket_image_render" not in code:
    # Careful, this pattern matches both ticket and reminder. We can replace both.
    code = code.replace(old_ticket_content_end, ticket_image_render + "\n" + old_ticket_content_end)
